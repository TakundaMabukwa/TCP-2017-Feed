...
CREATE TEMP TABLE wanted_waterford_plates (
  plate TEXT PRIMARY KEY
) ON COMMIT DROP;

INSERT INTO wanted_waterford_plates (plate)
VALUES
  ('LS38WYGP'),
  ('FM23CWGP'),
  ('FV70YVGP'),
...
WHERE evl.cost_code = 'WATE-0001'
  AND UPPER(COALESCE(evl.plate, '')) NOT IN (
    SELECT plate FROM wanted_waterford_plates
  );
...
FROM wanted_waterford_plates wanted
WHERE NOT EXISTS (
  SELECT 1
